...
class Lambda(nn.Module):
    ...
    def misha_2d(self, t, y):
        s = -y[0][1] + y[0][0] * (1 - y[0][0] ** 2 - y[0][1] ** 2)
        v = y[0][0] + y[0][1] * (1 - y[0][0] ** 2 - y[0][1] ** 2)

        return torch.tensor(np.array([s, v]), dtype=torch.float).reshape(1, 2)

    def forward(self, t, y):
        return self.misha_2d(t, y)
...
class ODEFunc(nn.Module):
    ...
    def forward(self, t, y):
        ...
        if len(y.shape) == 2:
            s = -y[:, 1] + y[:, 0] * (1 - y[:, 0] ** 2 - y[:, 1] ** 2)
            v = y[:, 0] + y[:, 1] * (1 - y[:, 0] ** 2 - y[:, 1] ** 2)

            s = s.reshape(-1, 1)
            v = v.reshape(-1, 1)
            a = torch.cat((s, v), dim=1)
        ...
        a = torch.cat((s, v), dim=1).reshape(-1, 1, 2)
